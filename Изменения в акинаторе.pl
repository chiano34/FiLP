...
problem(pc_smoked) :-
   query('Is your pc in smoke').

problem(display_blinking):-
   query('is your display blinking').

problem(pc_not_starting):-
   query('is your pc not starting').

fault(graphic_card_broken):-
    problem(pc_smoked),
    problem(pc_not_starting).

fault(display_hitted):-
    problem(display_blinking),
    problem(blank_display).
